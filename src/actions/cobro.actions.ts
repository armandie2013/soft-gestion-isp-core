...
export async function registrarPagoCobradorAction(
  ...
  const clienteId = String(formData.get("clienteId") || "");
  const facturaAsociadaId = String(
    formData.get("facturaAsociadaId") || "",
  );

  /*
   * El importe se conserva como texto hasta llegar al servicio.
   *
   * Ejemplo:
   * "25000.24"
   *
   * No se utiliza Number(...) en esta acción para evitar modificar
   * prematuramente el valor recibido antes de convertirlo a centavos.
   */
