...
const MovimientoFinancieroSchema = new Schema(
  {
    ...
    origenFacturacion: {
      type: String,
      enum: ["manual", "cron", "cobrador", "admin", "sistema"],
      default: "manual",
      index: true,
    },
...
MovimientoFinancieroSchema.pre("validate", function (next) {
  ...
  if (this.cicloFacturacionKey === null) {
    this.set("cicloFacturacionKey", undefined);
  }
...
MovimientoFinancieroSchema.index(
  { clienteId: 1, cicloFacturacionKey: 1 },
  {
    name: "cliente_ciclo_facturacion_unico_si_existe",
    unique: true,
    partialFilterExpression: {
      cicloFacturacionKey: {
        $exists: true,
