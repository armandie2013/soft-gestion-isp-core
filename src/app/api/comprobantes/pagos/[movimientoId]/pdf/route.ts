...
function renderComprobantePagoHtml({
  ...
  const config = getPageConfig(size);
  const clienteNombre = `${comprobante.clienteApellido}, ${comprobante.clienteNombre}`;
  const estaCorregido = Boolean(comprobante.estaCorregidoParcialmente);
  const importeVisible = estaCorregido
...
<body>
  <main class="receipt">
    <section class="header">
      ...
      <div class="status">${estaCorregido ? "Corregido parcialmente" : "Pago registrado"}</div>
    </section>

    <section class="amount-box">
      <p class="amount-label">${estaCorregido ? "Importe válido actual" : "Importe pagado"}</p>
      <p class="amount">${escapeHtml(formatMoney(importeVisible))}</p>
      ${
        estaCorregido
          ? `<p class="period">Registrado originalmente: ${escapeHtml(
              formatMoney(comprobante.importePagado),
            )} | Corrección: ${escapeHtml(
        ...
        ${
          estaCorregido
            ? `<div class="line">
                ...
                <span class="value">Corregido parcialmente</span>
              </div>
              <div class="line">
                <span class="label">Corrección</span>
                <span class="value">${escapeHtml(
    ...
    ${
      estaCorregido
        ? `
          <section class="observation">
            <p class="box-title">Comprobante corregido parcialmente</p>
            <p class="verify-text">Este comprobante fue registrado originalmente por ${escapeHtml(
              formatMoney(comprobante.importePagado),
            )}, pero su importe válido actual es ${escapeHtml(
              formatMoney(comprobante.importeValidoFinal),
            )}. La diferencia corregida es ${escapeHtml(
              formatMoney(comprobante.importeCorregido),
            )}.</p>
          </section>
        `
