        ...
        <section className="flex flex-1 items-center">
          <div className="w-full rounded-[1.7rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-900/80">
            <div className="flex flex-col items-center text-center">
              ...
              <h2 className="mt-2 text-2xl font-semibold">
                {resultado.valido
                  ? resultado.estaCorregidoParcialmente
            ...
            {resultado.valido ? (
              <>
                  ...
                  {resultado.estaCorregidoParcialmente ? (
                    <>
                      <DataLine
                        ...
                        value={formatMoney(resultado.importeValidoFinal)}
                      />

                      <DataLine
                        label="Corrección"
                        value={formatMoney(resultado.importeCorregido)}
                      />

                      <DataLine
                        label="Comp. corrección"
                        value={
                          resultado.comprobanteCorreccionNumero
                            ? `N° ${resultado.comprobanteCorreccionNumero}`
                            : "Registrada"
                        }
                      />
                    </>
                ...
                {resultado.estaCorregidoParcialmente ? (
                  <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-200">
                    Este comprobante existió, pero fue corregido parcialmente.
                    El importe válido actual es{" "}
                    {formatMoney(resultado.importeValidoFinal)}. No debe tomarse
                    como válido por el importe original completo.
                  </div>
                ) : null}
              </>
            ) : (
