...
export default async function ComprobantePagoPage({
  ...
  const [comprobante, user] = await Promise.all([
    obtenerComprobantePagoCliente(params.movimientoId),
    getCurrentUser(),
  ]);
  ...
  return (
    <PageShell maxWidth="md">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between print:hidden">
        ...
        {user?.rol === "admin" && !comprobante.estaCorregidoParcialmente ? (
          <Link
            href={`/admin/caja-cobradores/corregir-pago/${params.movimientoId}`}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 text-sm font-semibold text-red-800 shadow-sm transition hover:bg-red-100 active:scale-[0.99] dark:border-red-900/70 dark:bg-red-950/40 dark:text-red-300 dark:hover:bg-red-950/60"
          >
            Corregir pago
          </Link>
        ) : null}
      ...
      <div className="rounded-[1.7rem] border border-[var(--app-border)] bg-[var(--app-card)] p-4 shadow-[var(--app-shadow-soft)] sm:p-6">
        <div className="flex flex-col gap-4 border-b border-[var(--app-border)] pb-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Badge
            variant={comprobante.estaCorregidoParcialmente ? "warning" : "success"}
          >
            {comprobante.estaCorregidoParcialmente
        ...
        <div className="mt-5 rounded-2xl border border-emerald-200 bg-[var(--app-success-soft)] p-4 text-[var(--app-success)] dark:border-emerald-900/70">
          ...
          <p className="mt-2 text-4xl font-semibold tracking-tight">
            {formatMoney(
              comprobante.estaCorregidoParcialmente
                ? comprobante.importeValidoFinal
                : comprobante.importePagado,
            )}
          </p>

          {comprobante.estaCorregidoParcialmente ? (
            <p className="mt-2 text-sm opacity-90">
              Registrado originalmente: {formatMoney(comprobante.importePagado)}
              . Corrección aplicada: {formatMoney(comprobante.importeCorregido)}.
            </p>
          ) : null}
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          ...
          >
            <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-solid)] px-4 py-2">
              ...
              {comprobante.estaCorregidoParcialmente ? (
                <>
                  <DataLine label="Estado" value="Corregido parcialmente" />
                  <DataLine
                    label="Corrección"
                    value={
                      comprobante.comprobanteCorreccionNumero
                        ? `N° ${comprobante.comprobanteCorreccionNumero}`
                        : "Registrada"
                    }
                  />
                  <DataLine
                    label="Importe válido"
                    value={formatMoney(comprobante.importeValidoFinal)}
                  />
                </>
              ) : null}
              <DataLine label="Cobrador" value={comprobante.cobradorNombre} />
        ...
        {comprobante.estaCorregidoParcialmente ? (
          <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-200">
            <p className="text-xs font-semibold uppercase tracking-[0.16em]">
              Comprobante corregido parcialmente
            </p>

            <p className="mt-2 text-sm leading-6">
              Este comprobante fue registrado originalmente por{" "}
              {formatMoney(comprobante.importePagado)}, pero su importe válido
              actual es {formatMoney(comprobante.importeValidoFinal)}. La
              diferencia corregida es{" "}
              {formatMoney(comprobante.importeCorregido)}.
            </p>
          </div>
        ) : null}
