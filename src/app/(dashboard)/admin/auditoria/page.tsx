...
function ResultadoBadge({
  ...
  resultado: AuditLogSafe["resultado"];
}) {
  const config = {
    success:
    ...
    info: "border-blue-300 bg-blue-50 text-blue-700 dark:border-blue-900/70 dark:bg-blue-950/35 dark:text-blue-300",
  }[resultado];
  ...
  return (
    <span
      className={`inline-flex h-6 w-fit items-center rounded-full border px-2 text-[10px] font-semibold leading-none ${config}`}
    >
      {label}
    </span>
  );
...
function ResultadoIcon({ resultado }: { resultado: AuditLogSafe["resultado"] }) {
  if (resultado === "success") {
    return (
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-emerald-300 bg-emerald-50 text-emerald-700 dark:border-emerald-900/70 dark:bg-emerald-950/35 dark:text-emerald-300">
  ...
  if (resultado === "failure") {
    return (
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-red-300 bg-red-50 text-red-700 dark:border-red-900/70 dark:bg-red-950/35 dark:text-red-300">
  ...
  return (
    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-blue-300 bg-blue-50 text-blue-700 dark:border-blue-900/70 dark:bg-blue-950/35 dark:text-blue-300">
...
function FilterLink({
  ...
  return (
    <Link
      href={href}
      className={`inline-flex h-8 min-w-0 items-center justify-center rounded-lg border px-2.5 text-[12px] font-medium leading-none shadow-sm transition active:scale-[0.99] ${
        active
          ? "border-blue-600 bg-blue-600 !text-white shadow-blue-950/10 hover:border-blue-700 hover:bg-blue-700 dark:border-blue-500 dark:bg-blue-500 dark:!text-white dark:hover:border-blue-600 dark:hover:bg-blue-600"
          : "border-slate-300 bg-white text-slate-700 shadow-slate-300/30 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 dark:border-slate-700 dark:bg-slate-950/55 dark:text-slate-200 dark:shadow-black/10 dark:hover:border-blue-700 dark:hover:bg-blue-950/30 dark:hover:text-blue-300"
      }`}
    >
      <span
...
function HeaderAuditoria() {
  return (
    <section className={`${panelClass} p-3.5`}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <p className={sectionTitleClass}>Auditoría</p>

          <h1 className={sectionSubtitleClass}>Registros del sistema</h1>

          <p className={`${sectionDescriptionClass} max-w-3xl`}>
            Consulta de acciones sensibles, accesos, operaciones de caja, cobros
...
function FiltrosAuditoria({ filtroActivo }: { filtroActivo: string }) {
  return (
    <section className={`${panelClass} p-3.5`}>
...
function AuditDesktopTable({ logs }: { logs: AuditLogSafe[] }) {
  return (
    <section className={`${panelClass} hidden p-3.5 lg:block`}>
      <div className={innerPanelClass}>
        <div className="max-h-[640px] overflow-auto">
          <table className="min-w-full divide-y divide-slate-200 text-left text-[12px] dark:divide-slate-700">
            <thead className="sticky top-0 z-10 bg-slate-100/95 text-[10px] uppercase tracking-[0.08em] text-slate-500 backdrop-blur dark:bg-slate-950/95 dark:text-slate-400">
              <tr>
            ...
            <tbody className="divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-950/40">
              {logs.length === 0 ? (
                <tr>
                  <td
                    colSpan={7}
                    className="px-3 py-10 text-center text-[12px] text-slate-500 dark:text-slate-400"
                  >
                    No hay registros de auditoría para este filtro.
                  </td>
                </tr>
                ...
                logs.map((log) => (
                  <tr
                    key={log.id}
                    className="align-top transition hover:bg-blue-50/55 dark:hover:bg-blue-950/12"
                  >
                    <td className="whitespace-nowrap px-3 py-2.5 text-[12px] text-slate-600 dark:text-slate-400">
                      {formatDateTime(log.createdAt)}
                    </td>
                    ...
                    <td className="px-3 py-2.5">
                      <p className="max-w-[260px] text-[12px] leading-5 text-slate-700 dark:text-slate-300">
                        {log.mensaje || "-"}
                      </p>
                    </td>
                  </tr>
...
function AuditMobileList({ logs }: { logs: AuditLogSafe[] }) {
  if (logs.length === 0) {
    return (
      <section
        className={`${panelClass} p-4 text-[12px] leading-5 text-slate-600 dark:text-slate-400 lg:hidden`}
      >
        No hay registros de auditoría para este filtro.
      </section>
    );
    ...
    <section className="grid gap-2.5 lg:hidden">
      {logs.map((log) => (
        ...
        >
          ...
          <div className="mt-2 grid gap-2 rounded-lg border border-slate-300 bg-slate-50 p-3 text-[12px] shadow-sm shadow-slate-300/30 dark:border-slate-700 dark:bg-slate-950/50 dark:shadow-black/10">
            <div className="flex items-center justify-between gap-3">
              <span className="text-slate-500 dark:text-slate-400">Actor</span>
    ...
    <section className={`${panelClass} p-3.5`}>
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h2 className={sectionSubtitleClass}>Registros guardados</h2>
        </div>
...
function AlcanceAside() {
  return (
    <section className={`${panelClass} p-3.5`}>
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className={sectionTitleClass}>Alcance</p>

          <h2 className={sectionSubtitleClass}>Eventos registrados</h2>
        </div>
      ...
      <div className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2.5 text-[12px] leading-5 text-amber-800 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-300">
        Este panel muestra los últimos 120 registros. Más adelante podemos
        agregar búsqueda por usuario, IP, acción y rango de fechas.
      </div>
    </section>
  );
}

function AccesosAside() {
  return (
    <section className={`${panelClass} p-3.5`}>
      <div className="mb-3">
        <p className={sectionTitleClass}>Accesos</p>
      ...
      <div className="grid gap-2">
        ...
        >
          <span className="inline-flex items-center gap-2">
            <UserRound className="h-4 w-4 text-slate-500 transition group-hover:text-blue-700 dark:text-slate-400 dark:group-hover:text-blue-300" />
            Usuarios
          </span>
        ...
        >
          <span className="inline-flex items-center gap-2">
            <Activity className="h-4 w-4 text-slate-500 transition group-hover:text-blue-700 dark:text-slate-400 dark:group-hover:text-blue-300" />
            Caja cobradores
          </span>
    ...
    <PageShell maxWidth="wide" className="pb-20 sm:pb-0">
      <DashboardGrid>
        <DashboardMain>
          ...
          <div className="mt-3">
            <FiltrosAuditoria filtroActivo={filtroActivo} />
          </div>
