...
export function CopyDniButton({
  ...
  return (
    <button
      ...
      onClick={handleCopy}
      disabled={!dniValue}
      title={copied ? "DNI copiado" : "Copiar DNI"}
      aria-label={copied ? "DNI copiado" : "Copiar DNI"}
      className={`inline-flex items-center justify-center gap-1.5 rounded-lg border text-[10px] font-medium transition disabled:cursor-not-allowed disabled:opacity-50 active:scale-[0.99] ${
        iconOnly ? "h-7 w-7 px-0" : "h-7 px-2"
      } ${
        copied
          ? "border-emerald-300 bg-emerald-50 text-emerald-700 dark:border-emerald-900/70 dark:bg-emerald-950/40 dark:text-emerald-300"
          : "border-cyan-300 bg-cyan-50 text-cyan-800 hover:bg-cyan-100 dark:border-cyan-900/70 dark:bg-cyan-950/40 dark:text-cyan-300 dark:hover:bg-cyan-950/70"
    ...
    >
      {copied ? (
        <>
          ...
          {!iconOnly ? "Copiado" : null}
        </>
      ) : (
        <>
          <Copy className="h-3.5 w-3.5" />
          {!iconOnly ? "Copiar DNI" : null}
        </>
      )}
    </button>
  );
