...
} satisfies Record<UserRole, MobileLink[]>;
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => setOpen(true)}
        className="fixed bottom-4 right-4 z-40 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-blue-600 text-white shadow-lg active:scale-95 dark:bg-blue-500 lg:hidden"
        aria-label="Abrir menú"
      >
        <Menu className="h-5 w-5" />
      </button>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 max-h-[84vh] overflow-y-auto rounded-t-xl border border-slate-200 bg-white p-4 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div className="min-w-0">
              ...
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 dark:hover:bg-slate-800"
                aria-label="Cerrar menú"
              >
                <X className="h-4 w-4" />
              </button>
              ...
              {links.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setOpen(false)}
                    className={`flex items-center gap-3 rounded-lg border px-3 py-3 text-sm font-medium transition ${
                      active
