...
export function CorreccionPagoForm({ contexto }: CorreccionPagoFormProps) {
  ...
  const importeInicialCentavos = moneyNumberToCents(
    contexto.importeValidoActual || contexto.importeRegistrado,
  );
  const [importeRealIngresado, setImporteRealIngresado] = useState(
    `${Math.trunc(importeInicialCentavos / 100)},${String(
      importeInicialCentavos % 100,
    ).padStart(2, "0")}`,
  );

  const importeRealCentavos = useMemo(
    () => moneyInputToCents(importeRealIngresado),
    [importeRealIngresado],
  );
  const importeRealNumber = centsToMoney(importeRealCentavos);
  const importeRealVisual = formatCurrencyFromInput(importeRealIngresado);
  const importeRealBackend = moneyInputToBackendValue(importeRealIngresado);
  const importeRegistradoCentavos = moneyNumberToCents(contexto.importeRegistrado);
  const diferenciaCentavos = Math.max(
  ...
  const cajaSuficiente =
    contexto.cobroYaCerrado ||
    moneyNumberToCents(contexto.saldoCajaActual) >= diferenciaCentavos;
  const importeValido =
    importeRealCentavos >= 0 && importeRealCentavos < importeRegistradoCentavos;
  const puedeEnviar = contexto.puedeCorregir && importeValido && cajaSuficiente;
  ...
  function appendImporteCharacter(character: string) {
    setImporteRealIngresado((current) => {
      if (/^\d$/.test(character)) {
  ...
  function handleImporteKeyDown(event: KeyboardEvent<HTMLInputElement>) {
    const controls = ["Tab", "ArrowLeft", "ArrowRight", "Home", "End", "Escape", "Enter"];
    if (controls.includes(event.key) || event.ctrlKey || event.metaKey) return;
    if (event.key === "Backspace") {
      event.preventDefault();
      setImporteRealIngresado((current) => current.slice(0, -1));
      return;
    }
    if (event.key === "Delete") {
      event.preventDefault();
      setImporteRealIngresado("");
      return;
    }
  ...
  function handleImporteChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    setImporteRealIngresado(normalizarImportePegado(event.target.value));
  }
  ...
  return (
    <form action={formAction} className="space-y-3">
      <input type="hidden" name="movimientoId" value={contexto.movimientoId} />
      <input type="hidden" name="importeReal" value={importeRealBackend} />
      <input type="hidden" name="clienteId" value={contexto.clienteId} />
      <input type="hidden" name="cobradorId" value={contexto.cobradorId} />
      <input
        type="hidden"
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_340px]">
        <section className={`${panelClass} p-3.5`}>
          <div className="grid gap-3 lg:grid-cols-[minmax(240px,340px)_minmax(0,1fr)]">
            <div>
              <label htmlFor="importeRealVisual" className={labelClass}>
                Importe real recibido
              </label>

              <input
                id="importeRealVisual"
                type="text"
                inputMode="decimal"
                value={importeRealVisual}
              ...
              <p className="mt-1.5 text-[11px] leading-5 text-slate-500 dark:text-slate-400">
                Podés ingresar enteros o hasta dos centavos, por ejemplo $ 25.000,24.
              </p>
