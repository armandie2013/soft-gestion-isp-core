...
function getTipoInfo(tipo: string): TipoInfo {
  ...
  if (tipo === "ajuste_correccion_pago") {
    return {
      label: "Corrección",
      badge: "danger",
      icon: ArrowDownLeft,
  ...
  if (tipo === "ajuste_correccion_pago_post_cierre") {
    return {
      label: "Post-cierre",
      badge: "warning",
      icon: History,
...
export function CajaCobradorTable({ movimientos }: CajaCobradorTableProps) {
  ...
  return (
    <>
      <div className="hidden overflow-hidden rounded-[1.35rem] border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950/40 lg:block">
        <div className="grid grid-cols-[170px_160px_minmax(0,1fr)_130px_150px_150px_150px] border-b border-slate-200 bg-slate-50 px-4 py-3 text-[10px] font-medium uppercase tracking-[0.14em] text-slate-500 dark:border-slate-800 dark:bg-slate-950/70 dark:text-slate-400">
          <span>Fecha</span>
          <span>Tipo</span>
          <span>Descripción</span>
          <span>Estado</span>
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-800">
          {movimientos.map((movimiento) => {
            ...
            return (
              <div
                key={movimiento.id}
                className="grid grid-cols-[170px_160px_minmax(0,1fr)_130px_150px_150px_150px] items-center gap-3 px-4 py-3 transition hover:bg-slate-50 dark:hover:bg-slate-950/60"
              >
                <div className="min-w-0">
                  <p className="truncate text-xs font-medium text-slate-700 dark:text-slate-300">
                    {formatDate(movimiento.creadoEn)}
