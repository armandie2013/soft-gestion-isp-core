...
//           message:
//             "El importe debe ser válido y tener como máximo dos decimales.",
//         });
...
// export type RegistrarPagoCobradorInput = z.input<
//   typeof registrarPagoCobradorSchema
// >;
...
//     return (
//       Number(a.numeroComprobante || 0) -
//       Number(b.numeroComprobante || 0)
//     );
//   });
// }

// /**
//  * Convierte un importe almacenado en MongoDB a centavos enteros.
//  *
//  * Ejemplo:
//  * 25000.24 -> 2500024
//  *
//  * El toFixed(2) se usa únicamente para leer correctamente un Number
//  * ya almacenado. No se utiliza para eliminar los centavos ingresados.
//  */
...
// /**
//  * Convierte los centavos enteros al formato Number requerido por
//  * los modelos actuales de MongoDB.
//  *
//  * Ejemplo:
//  * 2500024 -> 25000.24
//  */
...
//   const limiteCajaCobrador = Number(
//     cobradorRaw.limiteCajaCobrador ??
//       LIMITE_CAJA_MINIMO,
//   );
...
//     await MovimientoFinanciero.findOne({
//       clienteId,
//     })
//       .sort({
//         fecha: -1,
...
// async function obtenerSaldoCajaCobrador(cobradorId: string) {
//   const ultimoMovimiento = await CajaCobrador.findOne({
//     cobradorId,
//   })
//     .sort({
//       creadoEn: -1,
...
// async function obtenerCierreCajaAsociado(
//   movimientoCaja: any,
// ) {
//   if (movimientoCaja?.cierreCajaId) {
//     const cierreDirecto = await CajaCobrador.findOne({
//       _id: movimientoCaja.cierreCajaId,
//       tipoMovimiento: "cierre_caja",
//     }).lean();
...
//     creadoEn: {
//       $gte:
//         movimientoCaja.cerradoEn ||
//         movimientoCaja.creadoEn,
//     },
//   })
//     .sort({
//       creadoEn: 1,
...
//   const movimientosRaw = await CajaCobrador.find({
//     cobradorId,
//   })
//     .sort({
//       creadoEn: -1,
...
//   const movimientosBase = movimientosRaw.map(
//     toSafeCajaMovimiento,
//   );

//   const movimientos = await enriquecerMovimientosCaja(
//     movimientosBase,
//   );

//   const totalCobradoCentavos = movimientos
//     .filter(
//       (movimiento) =>
...
//     .reduce(
//       (acc, movimiento) =>
//         acc + moneyToCents(movimiento.importe),
//       0,
//     );

//   const totalCierresCentavos = movimientos
//     .filter(
//       (movimiento) =>
//         movimiento.tipoMovimiento === "cierre_caja",
//     )
//     .reduce(
//       (acc, movimiento) =>
//         acc + moneyToCents(movimiento.importe),
...
//   const totalAjustesCorreccionCentavos = movimientos
//     .filter(
//       (movimiento) =>
...
//   const totalAjustesPostCierreCentavos = movimientos
//     .filter(
//       (movimiento) =>
//         movimiento.tipoMovimiento ===
//         "ajuste_correccion_pago_post_cierre",
...
//     totalCierresCentavos -
//     totalAjustesCorreccionCentavos -
//     totalAjustesPostCierreCentavos;

//   return {
//     totalCobrado: centsToMoney(
//       totalCobradoCentavos,
//     ),

//     totalCierres: centsToMoney(
//       totalCierresCentavos,
//     ),

//     totalAjustesCorreccion: centsToMoney(
//       totalAjustesCorreccionCentavos,
//     ),

//     totalAjustesPostCierre: centsToMoney(
//       totalAjustesPostCierreCentavos,
//     ),

//     saldoActual: centsToMoney(
//       saldoActualCentavos,
//     ),
...
//   const limiteCajaCobrador = Math.max(
//     Number(
//       datosCajaCobrador.limiteCajaCobrador ||
//         LIMITE_CAJA_MINIMO,
//     ),
//     LIMITE_CAJA_MINIMO,
//   );
...
//     saldoCajaActual,
//     limiteCajaCobrador,
//     disponibleCaja: Math.max(
//       limiteCajaCobrador - saldoCajaActual,
//       0,
//     ),
//   };
// }
...
//   const resultados = await Promise.all(
//     cobradores.map(async (cobrador: any) => {
//       const caja = await obtenerCajaCobradorResumen(
//         cobrador._id.toString(),
//       );

//       const codigoPendienteRaw =
//         await CodigoCierreCaja.findOne({
//           cobradorId: cobrador._id,
//           estado: "pendiente",
//           venceEn: {
//             $gte: new Date(),
...
//         totalCierres: caja.totalCierres,
//         totalAjustesCorreccion:
//           caja.totalAjustesCorreccion,
//         totalAjustesPostCierre:
//           caja.totalAjustesPostCierre,
//         codigoPendiente: codigoPendienteRaw
//           ? toSafeCodigo(codigoPendienteRaw)
//           : null,
//       };
...
//   const cobradores =
//     await obtenerCobradoresCajaResumen();

//   const totalSaldoCobradores = cobradores.reduce(
//     (acc, cobrador) =>
//       acc + cobrador.saldoActual,
//     0,
//   );

//   const totalRecibidoAdmin = cobradores.reduce(
//     (acc, cobrador) =>
//       acc + cobrador.totalCierres,
//     0,
//   );

//   const totalAjustesPostCierre = cobradores.reduce(
//     (acc, cobrador) =>
//       acc + cobrador.totalAjustesPostCierre,
//     0,
//   );

//   const cantidadCobradoresConSaldo =
//     cobradores.filter(
//       (cobrador) => cobrador.saldoActual > 0,
//     ).length;

//   const cantidadCodigosPendientes =
//     cobradores.filter(
//       (cobrador) => cobrador.codigoPendiente,
//     ).length;

//   const cantidadCierresRealizados =
//     cobradores.reduce(
//       (acc, cobrador) =>
//         acc +
//         (cobrador.totalCierres > 0 ? 1 : 0),
//       0,
//     );
...
// ) {
//   const parsed =
//     registrarPagoCobradorSchema.safeParse(input);
...
//   /*
//    * El importe validado llega expresado como
//    * centavos enteros.
//    *
//    * Ejemplo:
//    * $25.000,24 = 2.500.024 centavos.
//    */
//   const importe = centsToMoney(
//     importeCentavos,
//   );
...
//   const cliente =
//     await Cliente.findById(clienteId).lean();
...
//     await obtenerDetallePeriodoCliente(
//       clienteId,
//       facturaAsociadaId,
//     );
...
//       ok: false,
//       message:
//         "El período seleccionado no existe o no pertenece al cliente.",
//     };
//   }

//   const estadoCuenta =
//     await obtenerEstadoCuentaCliente(clienteId);
...
//   const periodosPendientesOrdenados =
//     ordenarPeriodosPendientesPorAntiguedad(
//       estadoCuenta.periodos.filter(
//         (periodo) =>
//           periodo.saldoPeriodo > 0,
//       ),
//     );

//   const primerPeriodoPendiente =
//     periodosPendientesOrdenados[0] || null;

//   if (
//     primerPeriodoPendiente &&
//     primerPeriodoPendiente.facturaId !==
//       facturaAsociadaId
//   ) {
//     return {
//       ok: false,
...
//   if (
//     detallePeriodo.periodo.saldoPeriodo <= 0
//   ) {
//     return {
//       ok: false,
//       message:
//         "El período seleccionado no tiene saldo pendiente.",
//     };
//   }

//   const saldoPeriodoCentavos = moneyToCents(
//     detallePeriodo.periodo.saldoPeriodo,
//   );

//   if (
//     importeCentavos >
//     saldoPeriodoCentavos
//   ) {
//     return {
//       ok: false,
//       message: `El pago no puede superar el saldo del período (${formatMoney(
...
//     datosCajaCobrador,
//   ] = await Promise.all([
//     obtenerSaldoCajaCobrador(
//       cobrador.userId,
//     ),
//     obtenerDatosCajaCobrador(
//       cobrador.userId,
//     ),
//   ]);
...
//       ok: false,
//       message:
//         "Cobrador no encontrado o sin permisos para registrar pagos.",
//     };
//   }

//   const { limiteCajaCobrador } =
//     datosCajaCobrador;

//   const saldoActualCajaCentavos =
//     moneyToCents(saldoActualCaja);

//   const limiteCajaCentavos =
//     moneyToCents(limiteCajaCobrador);

//   const saldoCajaProyectadoCentavos =
//     saldoActualCajaCentavos +
//     importeCentavos;

//   if (
//     saldoCajaProyectadoCentavos >
//     limiteCajaCentavos
//   ) {
//     return {
//       ok: false,
...
//   const saldoActualCliente =
//     await obtenerSaldoActualCliente(
//       clienteId,
//     );

//   /*
//    * La resta también se realiza usando
//    * centavos enteros.
//    *
//    * De esta manera, un cobro de $25.000,24
//    * conserva exactamente los 24 centavos.
//    */
//   const nuevoSaldoClienteCentavos =
//     moneyToCents(saldoActualCliente) -
//     importeCentavos;

//   const nuevoSaldoCliente = centsToMoney(
//     nuevoSaldoClienteCentavos,
//   );

//   const numeroComprobante =
//     await obtenerSiguienteNumeroComprobante();

//   const concepto =
//     `Pago período ${detallePeriodo.periodo.periodoLabel}` +
//     ` - Factura N° ${detallePeriodo.periodo.numeroComprobante}`;

//   const movimientoFinancieroId =
...
//   const firmaVerificacion =
//     generarFirmaPago({
//       movimientoId:
//         movimientoFinancieroId.toString(),
//       numeroComprobante,
//       clienteId,
//       clienteDni:
//         cliente.dni || "",
...
//   const nuevoSaldoCaja = centsToMoney(
//     saldoCajaProyectadoCentavos,
//   );
...
//     saldoCaja: nuevoSaldoCaja,
//     descripcion:
//       `Cobro a cliente ${clienteNombre} ` +
//       `DNI ${clienteDni} - ${concepto}`,
//     observacion:
//       observacion?.trim() || "",
//   });
...
//     movimientoId:
//       movimientoFinanciero._id.toString(),
//     numeroComprobante,
//   };
// }
...
//   const saldoCajaActual = cobradorId
//     ? await obtenerSaldoCajaCobrador(
//         cobradorId,
//       )
//     : 0;

//   const cierreAsociado = movimientoCaja
//     ? await obtenerCierreCajaAsociado(
//         movimientoCaja,
//       )
//     : null;

//   const estadoCajaMovimiento =
//     movimientoCaja
//       ? getEstadoCajaMovimiento(
//           movimientoCaja,
//         )
//       : "abierto";

//   const cobroYaCerrado =
//     estadoCajaMovimiento === "cerrado" ||
//     Boolean(cierreAsociado);

//   const importeRegistrado = Number(
//     pago.haber || 0,
//   );

//   const importeYaCorregido = Number(
//     pago.importeCorregido || 0,
//   );

//   const importeValidoActual =
//     pago.importeValidoFinal !== null &&
//     pago.importeValidoFinal !== undefined
//       ? Number(
//           pago.importeValidoFinal || 0,
//         )
//       : importeRegistrado -
//         importeYaCorregido;
...
//   if (
//     correccionExistente ||
//     pago.estadoComprobante ===
//       "corregido_parcialmente"
//   ) {
//     puedeCorregir = false;
//     motivoNoPuedeCorregir =
//       "Este comprobante ya tiene una corrección asociada.";
...
//     puedeCorregir = false;
//     motivoNoPuedeCorregir =
//       "No se pudo identificar el cobrador del pago.";
//   }
...
//     facturaAsociadaId:
//       pago.facturaAsociadaId.toString(),
...
//     cierreCajaFecha:
//       cierreAsociado?.creadoEn
//         ?.toISOString?.() || null,

//     correccionImpactaCajaActual: true,

//     correccionImpactaCierreAnterior:
//       cobroYaCerrado,
...
//   const importeRegistradoCentavos = moneyToCents(importeRegistrado);
//   const importeRealCentavos = moneyToCents(importeRealNormalizado);

//   const diferenciaCentavos =
//     importeRegistradoCentavos - importeRealCentavos;
...
//   const diferencia = centsToMoney(diferenciaCentavos);

//   const saldoCajaActualCentavos = moneyToCents(
//     contexto.saldoCajaActual,
//   );

//   if (
//     !contexto.cobroYaCerrado &&
//     saldoCajaActualCentavos < diferenciaCentavos
//   ) {
//     return {
//       ok: false,
//       message:
...
//   const nuevoSaldoClienteCentavos =
//     moneyToCents(saldoActualCliente) + diferenciaCentavos;

//   const nuevoSaldoCliente = centsToMoney(
//     nuevoSaldoClienteCentavos,
//   );

//   const saldoCajaActual = await obtenerSaldoCajaCobrador(
//     contexto.cobradorId,
//   );

//   const nuevoSaldoCajaCentavos =
//     moneyToCents(saldoCajaActual) - diferenciaCentavos;

//   const nuevoSaldoCaja = centsToMoney(
//     nuevoSaldoCajaCentavos,
//   );

//   const numeroComprobanteCorreccion =
//     await obtenerSiguienteNumeroComprobante();

//   const fechaCorreccion = new Date();

//   const concepto =
//     `Corrección de pago N° ${contexto.numeroComprobante}` +
//     " - importe registrado en exceso";

//   const observacionBase =
//     `${motivo.trim()} | ` +
//     `Importe registrado: ${formatMoney(importeRegistrado)}. ` +
//     `Importe real recibido: ${formatMoney(importeRealNormalizado)}. ` +
//     `Diferencia corregida: ${formatMoney(diferencia)}.`;

//   const observacion = cobroYaCerrado
//     ? `${observacionBase} Corrección posterior al cierre de caja. ` +
//       "Se descuenta como saldo pendiente de la próxima caja del cobrador."
//     : `${observacionBase} Corrección aplicada sobre caja abierta del cobrador.`;
...
//     cierreCajaId: cobroYaCerrado
//       ? cierreAsociado?._id || null
//       : null,
...
//   if (moneyToCents(caja.saldoActual) <= 0) {
//     return {
//       ok: false,
//       message: "El cobrador no tiene saldo pendiente para cerrar.",
//       codigo: null,
...
//     importe: centsToMoney(
//       moneyToCents(caja.saldoActual),
//     ),
...
//       ok: false,
//       message:
//         "Código incorrecto, vencido o no autorizado para este cobrador.",
//       importe: 0,
//     };
//   }
...
//   const importeCodigoCentavos = moneyToCents(
//     Number(codigo.importe || 0),
//   );

//   const saldoCajaCentavos = moneyToCents(
//     caja.saldoActual,
//   );

//   if (importeCodigoCentavos !== saldoCajaCentavos) {
//     return {
//       ok: false,
//       message:
//         "El importe de tu caja cambió desde que se generó el código. Pedí un nuevo código al administrador.",
...
//     importe: centsToMoney(saldoCajaCentavos),
//   };
// }
...
//   cobrador: UsuarioCobradorPago,
// ) {
//   const validacion = await validarCodigoCierreCajaCobrador(
//     input,
//     cobrador,
//   );
...
//   if (saldoCajaCentavos <= 0) {
//     return {
//       ok: false,
//       message: "Tu caja ya se encuentra en $ 0,00.",
//       importe: 0,
//     };
//   }

//   const saldoCaja = centsToMoney(
//     saldoCajaCentavos,
//   );
...
//     descripcion:
//       `Cierre de caja autorizado con código ${codigo.codigo}`,

//     observacion:
//       `Código generado por ${codigo.generadoPorAdminNombre}`,
//   });
...
//     ok: true,
//     message:
//       "Cierre de caja confirmado correctamente. Tu caja quedó en $ 0,00.",
//     importe: saldoCaja,
//   };
// }
  ...
  importeReal: z
    ...
    .transform((value, context) => {
      ...
      if (!/^\d+(?:\.\d{1,2})?$/.test(raw)) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message: "El importe real debe ser válido y tener como máximo dos decimales.",
        });
        return z.NEVER;
      }
      ...
      if (!Number.isSafeInteger(cents) || cents < 0) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message: "El importe real no puede ser negativo.",
        });
        return z.NEVER;
      }
...
export type CorregirPagoCobradorInput = z.input<
  typeof corregirPagoCobradorSchema
>;
...
export async function corregirPagoCobrador(
  ...
  const { movimientoId, importeReal: importeRealCentavos, motivo } = parsed.data;
  ...
  const importeRegistrado = Number(contexto.importeRegistrado || 0);
  const importeRegistradoCentavos = moneyToCents(importeRegistrado);
  const importeRealNormalizado = centsToMoney(importeRealCentavos);

  if (importeRealCentavos >= importeRegistradoCentavos) {
    return {
      ok: false,
      message:
        "El importe real debe ser menor al importe registrado originalmente.",
